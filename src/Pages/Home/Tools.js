    ...
    return (
        <div>
            ...
            <div className='lg:grid lg:grid-cols-3 gap-4 lg:place-content-evenly'>
                {
                    tools?.slice(-6).map(tool => <div key={tool._id} className="card lg:max-w-lg bg-base-100 shadow-xl">
                        <figure className="px-10 pt-10">
                            <img src={tool.img} alt={tool.name} className="rounded-xl hover:rotate-90 hover:ease-in duration-300" />
                        </figure>
                        <div className="card-body hover:scale-110 hover:ease-in duration-300 items-center text-center">
                            ...
                            <div className="card-actions">
                                <button onClick={() => handlePurchase(tool?._id)} className="btn btn-primary hover:rounded-full">Purchase Now</button>
                            </div>
                        </div>
                    </div>)
                }
